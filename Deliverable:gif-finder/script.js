...
function sendApiRequest(lol) {
  
    fetch("https://api.giphy.com/v1/gifs/search?api_key=bEjexG2XqqPE12HfB77vKoxr3n8xJakV&q="+lol+"&limit=25&offset=0&rating=G&lang=en")
    .then(function(data) {
      return data.json();
    })
    .then(function(json) {
...
function getImageURLfrom(myJSON) {
  const imageURL = myJSON.data[Math.floor(Math.random() * 25)].images.original.url 
